...
namespace Opc.Ua
{
    ...
    public class DataValue : IFormattable
    {
        ...
        public String GetStringValue(int wDataType)
        {
            ...
            switch (wDataType)
            {
                ...
                case 4:   //float
                    if (Value is float)
                    {
                        float wResultF = (float)Value;
                        if (wResultF > 10000000 || wResultF < -10000000 || (wResultF < 0.0000001 && wResultF > -0.0000001))
                        {
                            Byte[] wArray = BitConverter.GetBytes(wResultF);

                            Array.Reverse(wArray);
                            wResultF = BitConverter.ToSingle(wArray, 0);
                        }
                        return wResultF.ToString();
                    }
                    else if (Value is int)
                    {
                        float wResultF = 0;
                        Byte[] wArray = BitConverter.GetBytes((int)Value);
                ...
                case 5: //double
                    if (Value is Array && ((Array)Value).Length == 8)
                    {
                        Byte[] wArray = (Byte[])Value;
                        //默认double反转
                        Array.Reverse(wArray);

                        double wResultD = BitConverter.ToDouble(wArray, 0);

                        if (wResultD > 10000000 || wResultD < -10000000 || (wResultD < 0.0000001 && wResultD > -0.0000001))
                        {
                            Array.Reverse(wArray);
                            wResultD = BitConverter.ToDouble(wArray, 0);
                        }

                        return wResultD.ToString();
                    }
                    else
                    {
                        return Value.ToString();
        ...
        public Object GetObjectValue(int wDataType)
        { 
            if (Value == null)
            {
                return "";
